...
module.exports.validatePurchase = function (receipt, cb) {
	if (typeof receipt !== 'object') {
		return cb(new ValidationError('malformed receipt: ' + receipt, constants.VALIDATION.FAILURE));
	}
	if (!receipt.data || !receipt.signature) {
		return cb(new ValidationError('missing receipt data:\n' + JSON.stringify(receipt), constants.VALIDATION.FAILURE));
	}
	// try live first
...
function validatePublicKey(receipt, pkey, cb) {
	...
	// failed to validate
	cb(new ValidationError('failed to validate purchase', constants.VALIDATION.FAILURE));
}
